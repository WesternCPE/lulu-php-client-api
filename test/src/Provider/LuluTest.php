...
namespace WesternCPE\OAuth2\Client\Test\Provider;

use WesternCPE\OAuth2\Client\Provider\Cognito;
use WesternCPE\OAuth2\Client\Provider\CognitoUser;
...
class LuluTest extends TestCase {
	...
	protected function setUp(): void {
		$this->provider = new \WesternCPE\OAuth2\Client\Provider\Lulu(
			array(
	...
	//  );
	//  $provider = new \WesternCPE\OAuth2\Client\Provider\Cognito(
	//      array(
	//          'clientId'     => 'mock_client_id',
	//          'clientSecret' => 'mock_secret',
	//          'redirectUri'  => 'none',
	...
	// public function testNoHostedDomainNorLuluDomainButRegion() {
	//  $this->expectException( '\InvalidArgumentException' );
	//  $this->expectExceptionMessage(
	//      'Neither "cognitoDomain" and "region" nor "hostedDomain" options are set. Please set one of them.'
	//  );
	//  $provider = new \CakeDC\OAuth2\Client\Provider\Lulu(
	//      array(
	//          'clientId'     => 'mock_client_id',
	//          'clientSecret' => 'mock_secret',
	//          'redirectUri'  => 'none',
	...
	// public function testNoHostedDomainNorRegionButLuluDomain() {
	//  $this->expectException( '\InvalidArgumentException' );
	//  $this->expectExceptionMessage(
	//      'Neither "cognitoDomain" and "region" nor "hostedDomain" options are set. Please set one of them.'
	//  );
	...
	//      )
	//  );
	//  $this->assertEquals( array( 'test-scope', 'test-scope-2' ), $provider->getScopes() );
	// }
	...
	// public function testSetLuluDomainAfterConfig() {
	//  $host = uniqid();

	//  $this->provider->setLuluDomain( $host );

	//  $this->assertEquals( $host, $this->provider->getLuluDomain() );
	// }
	...
	// public function testSetLuluDomainAndRegionInConfig() {
	//  $host   = uniqid();
	//  $region = uniqid();
	...
	//  $this->assertEquals( $host, $provider->getLuluDomain() );
	//  $this->assertEquals( $region, $provider->getRegion() );
	// }

	// public function testScopes() {
	//  $scopeSeparator = ' ';
	//  $options        = array( 'scope' => array( uniqid(), uniqid() ) );
	//  $query          = array( 'scope' => implode( $scopeSeparator, $options['scope'] ) );
	...
	//      );
	//  $response->shouldReceive( 'getHeader' )->andReturn( array( 'content-type' => 'json' ) );
	...
	//      );
	//  $userResponse->shouldReceive( 'getHeader' )->andReturn( array( 'content-type' => 'json' ) );
